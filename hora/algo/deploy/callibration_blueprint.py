...
class Allegro(object):
    ...
    def __init__(self, hand_topic_prefix="allegroHand_0", num_joints=16):
        ...
        # Topics (that can be remapped) for named grasp (ready/envelop/grasp/etc.),
        # joint commands (position and velocity), joint state (subscribing),
        # and envelop torque. Note that we can change the hand topic prefix (for example, to allegroHand_0)
        ...
        # self.pub_envelop_torque = rospy.Publisher(topic_envelop_torque, Float32, queue_size=1)
        ...
        # "Named" grasps are those provided by the bhand library.
        # These can be commanded directly and the hand will execute them.
        # The keys are more human-friendly names, the values are the expected names
        # from the allegro controller side.
        # Multiple strings mapping to the same value are allowed.
    ...
    def command_joint_position(self, desired_pose):
        ...
        try:
            ...
            # rospy.logdebug("Published desired pose: {}".format(desired_pose))
            # print("Published desired pose: {}".format(msg.position))
            return True
    ...
    def command_joint_torques(self, desired_torques):
        ...
        Similarly to poses, we do not sanity-check the inputs.
        As a rule of thumb, values between +- 0.5 are fine.
        ...
        # and that the number of dimensions is the same as the number of joints.
        # This prevents passing singletons or incorrectly-shaped lists
        ...
        msg = JointState()  # Create and publish # https://docs.ros.org/en/noetic/api/sensor_msgs/html/msg/JointState.html
...
class HardwarePlayerCalib(object):
    def __init__(self):
        ...
        self.data_lock = threading.Lock()  # For thread-safe access to the buffers
    ...
    def control_loop(self):
        ...
        # It should be called in the main thread.
        import rospy
        ...
        # Command the initial position for setup (using home_pose)
        for t in range(hz * 8):
        ...
        try:
            while not rospy.is_shutdown() and self.running:
                ...
                # Generate a sinusoidal value between -1 and 1
                sin_val = np.sin(elapsed)
                # Start with the home_pose as baseline
                commands = np.array(self.home_pose)

                # TODO the pattern cycle is depending on the sinusoidal period. not absolute time
                # Alternate command patterns every 10 seconds:
                # For the first 5 seconds, only joint index 3 moves.
                # For the next 5 seconds, joints indices 3, 4, and 5 move.
                if (int(elapsed) % 10) < 5:
                    idx = 4
                ...
                # Append new data to the deques (thread safe)
                with self.data_lock:
